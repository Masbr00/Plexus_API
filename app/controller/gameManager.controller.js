...
exports.playerAchievment = async (req, res) => {
    try {
        const User_id = req.params.user_id
        var achievment = await sequelize.query(`select tb_users.name, tb_achievements.achievement from tb_users, tb_achievements, tb_listachievmentusers where tb_achievements.id = tb_listachievmentusers.achievment_id and tb_listachievmentusers.player_id = tb_users.id group by tb_achievements.achievement`)
